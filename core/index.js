...
export async function runLoop({ project, commit = 'HEAD', impact = null, maxFlows = Infinity, target: targetOverride = null, logger = createLogger() } = {}) {
  const root = resolveRoot(project)
  loadKodaEnv(root)
  const config = readConfig(root)
  const target = resolveTarget(config, targetOverride)
  ...
  if (!health.up) {
    logger.warn(`${target} is unavailable; browser and endpoint checks skipped.`)
    browserResults = skippedBrowser(agentContext.affected_flows, target)
    endpointResults = skippedEndpoints(agentContext.affected_endpoints, target)
  } else {
    if (config.tests.browser_flows) {
      const kane = await runKane(root, agentContext.affected_flows, target, config.kane, maxFlows)
      browserResults = kane.results
      selected = kane.selected
      requested = kane.requested
    }
    if (config.tests.endpoint_tests) endpointResults = await testEndpoints(root, agentContext.affected_endpoints, target, config)
  }
